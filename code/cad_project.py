...
def nuclei_classification():
    ...
    # plus 1 by 1. (nr columns+1,1) 
    
    nr_columns=np.shape(training_x)[1] #1728
    
    shapex=sum([nr_columns,1]) #1729
    
    #using different Theta inputs
    #Theta = np.zeros(shapex,1) #zeros
    Theta = np.random.rand(shapex, 1)#random between 0 and 1
